      ...
      <div className="min-h-screen bg-[#F8FAFC] text-slate-900">
        <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
          <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 py-2 sm:px-6 md:px-10">
            ...
            <nav className="hidden items-center gap-1 text-xs font-semibold text-slate-600 lg:flex" aria-label="Workspace switcher">
              <WorkspaceLink to="/" label="Public site" />
              <WorkspaceLink to="/dashboard" label="Candidate portal" />
              <WorkspaceLink to="/consultant-dashboard" label="Consultant portal" />
              <WorkspaceLink to="/employee-dashboard/applications" label="Operations portal" active />
            </nav>
            <div className="flex items-center gap-2">
              ...
              <Link to="/" className="inline-flex items-center gap-2 text-sm font-medium text-[#2563EB]">
        ...
        <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 md:px-10 md:py-8">
          <section className="mb-6 grid grid-cols-1 gap-4 xl:grid-cols-[0.85fr_1.15fr]">
            <div className="rounded-2xl bg-[#0A192F] p-6 text-white md:p-7">
              ...
              <h1 className="mt-4 font-heading text-3xl font-semibold tracking-tight text-white">
                Role-based queues for hiring, onboarding, and consultant operations.
              </h1>
              <p className="mt-3 text-sm leading-relaxed text-white/65">
                Signed in as {user?.email || "operations user"}. Use the section tabs to work one queue at a time.
              </p>
            </div>
          ...
          <nav className="mb-6 overflow-x-auto rounded-xl border border-slate-200 bg-white p-2" aria-label="Operations sections">
            <div className="flex min-w-max gap-2">
              {OPS_NAV.map(({ to, label, Icon }) => (
                <NavLink
                  key={to}
                  to={to}
                  className={({ isActive }) =>
          ...
          {error && <InlineError title="Operations data did not load" body={error} onRetry={load} className="mb-5" />}
          {loading && !data.applications.length ? (
            <LoadingState label="Loading operations queues..." />
          ) : (
            <Outlet />
          )}
        </main>
...
function WorkspaceLink({ to, label, active = false }) {
  return (
    <Link
      to={to}
      className={`rounded-md px-2.5 py-1.5 ${
        active ? "bg-[#2563EB]/10 text-[#1D4ED8]" : "hover:bg-slate-100 hover:text-slate-900"
      }`}
    >
      {label}
    </Link>
